...
export class FilterOrderComponent implements OnInit {
  ...
  public saveData() {
    ...
    for (let i = 0; i < this.backupItem.length; i++) {
      ...
      this._dataService.put('/api/order/update', this.backupItem[i]).subscribe((response: any) => {
        this.search();
        console.log(response);
      });
      this._dataService.post('/api/orderBackup/add', this.backupItem[i]).subscribe((response: any) => {
      }, error => this._dataService.handleError(error));
